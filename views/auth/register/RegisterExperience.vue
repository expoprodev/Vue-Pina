<template>
  <div class="login">
    <div class="container-fluid">
      <div class="row h-100vh">
        <DescriptionPanel>
          We can say something about <span>Digital Physics</span> here, and add a graphical element to show
          how CFD works.
        </DescriptionPanel>
        <div class="col-lg-6 h-100vh gx-0">
          <div class="experience-content h-100vh d-flex justify-content-center flex-column py-30">
            <div class="title">How experience are you with the models (Computational Fluid Dynamics, thermodynamics,
              molecular dynamics, etc.) you want to execute?
            </div>
            <form action="" v-on:submit.prevent="onFinishClick">

              <CheckBoxContainer v-model="store.experiences"/>

              <div v-if="store.error">
                {{ store.error }}
              </div>

              <div class="btn-container">
                <Button :disabled="store.isLoading" text="Finish"/>
                <Button :is-next-button="false" text="Back" type="button" @click="onPrevClick"/>
              </div>
            </form>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import router from "@/router";
import DescriptionPanel from "@/components/auth/DescriptionPanel.vue";
import CheckBoxContainer from "@/components/common/CheckBoxContainer.vue";
import Button from "@/components/common/Button.vue";
import {onMounted} from "vue";
import {useRegisterToolsAndExperienceStore} from "@/store/auth/registerToolsAndExperience.js";

export default {
  name: 'RegisterExperience',
  components: {
    DescriptionPanel,
    CheckBoxContainer,
    Button,
  },
  setup() {
    const store = useRegisterToolsAndExperienceStore()
    store.init()

    onMounted(() => {
      if (!store.email) {
        return router.push({name: "RegisterInformation"});
      }
    })

    const onPrevClick = () => {
      router.go(-1)
    }

    const onFinishClick = () => {
      store.completeRegister().then(() => {
        router.push({name: "Login"})
      }).catch(() => {
      })
    }

    return {
      store,
      onPrevClick,
      onFinishClick,
    }
  },
}
</script>

<!-- Add "scoped" attribute to limit CSS to this component only -->
<style scoped>

</style>
